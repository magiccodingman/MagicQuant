...
/// <summary>
/// Coordinates baseline learning, isolation measurements, prediction-guided selection,
/// real benchmark validation, and final export. Numerical policy lives in services.
/// </summary>
public class QuantizationPipeline : ICommand
{
    ...
    public async Task Run(List<CliArg> args)
    {
        ...
        AnsiConsole.Write(new Rule("[yellow]Pipeline Configuration[/]") { Justification = Justify.Left });
        ...
        // remainder of the pipeline run. Re-assert them here as a final safeguard.
        RuntimeSearchSpace.SetImatrixAvailability(imatrixEnsureResult.Enabled);
        RuntimeSearchSpace.AllowHighPrecisionHybrids = Config.Current.Flags.AllowHighPrecisionHybrids;
        ...
        AnsiConsole.MarkupLine("[bold magenta]Pipeline progress:[/] startup sampling finished. Learned-baseline early pruning remains disabled for subsequent phases.");
    ...
    {
        AnsiConsole.MarkupLine("[bold yellow]Command: pipeline (legacy alias: evolution)[/]");
        ...
        AnsiConsole.WriteLine("  mq pipeline --model-dir \"<path>\" [options]");
        AnsiConsole.WriteLine("  mq pipeline --config \"./config.default.yaml\"");
        AnsiConsole.WriteLine();
        ...
        AnsiConsole.MarkupLine("  [green]--allow-eight-bit-anchor-replacements[/]    Permit final prediction to try replacing 8-bit anchors like Q8_0 (see YAML policy)");
        ...
        AnsiConsole.WriteLine("  mq pipeline --model-dir \"C:\\Models\\Mistral-7B\"");
    }

    private static string ResolveAndValidateOutputDirectory()
    {
        string resolved = OutputPathService.Pipeline(
            Cache.ModelMagicQuantDirectory!, Config.Current.Output.OutputDir);
