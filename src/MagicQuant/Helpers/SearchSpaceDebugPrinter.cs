        ...
        foreach (var baseline in activeBaselines)
        {
            ...
            for (int i = 0; i < TReg.All.Length; i++)
            {
                ...
                if (MagicQuantDiagnostics.ShouldLogGroup(group))
                {
                    ...
                    var reasonMap = RuntimeSearchSpace.GetRuntimeExplicitCandidateBanReasonsForGroup(group);
                    ...
                    {
                        var grouped = runtimeBanned
                            .GroupBy(x => reasonMap.TryGetValue(x.UniqueId, out var r) ? r : "unspecified")
                            .Select(g => $"{g.Key}: {string.Join(", ", g.Select(x => x.Names[0]))}");
                        MagicQuantDiagnostics.Log("search-space", $"group={group.Name} restrictionReasons={string.Join(" | ", grouped)}");
                    }
