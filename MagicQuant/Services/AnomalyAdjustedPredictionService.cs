...
WHERE BaseRankSafeKld IS NOT NULL;", ct);
        ...
        foreach (var rule in rules.OrderByDescending(x => x.Confidence).ThenBy(x => x.Id))
        {
            ...
            string expression = adjustment < 0d
                ? $"GREATEST(COALESCE(AnomalyAdjustmentKld, 0.0) + ({SqlDouble(adjustment)}), -LEAST({SqlDouble(Math.Min(Config.AnomalyDetection.MaxNegativeAdjustmentKld, Config.SynergyDetection.MaxNegativeAdjustmentKld))}, COALESCE(BaseRankSafeKld, 0.0) * {SqlDouble(Math.Min(Config.AnomalyDetection.MaxAdjustmentFractionOfBaseKld, Config.SynergyDetection.MaxNegativeAdjustmentFractionOfBaseKld))}))"
...
WHERE {where};", ct);
            ...
            var log = new
            {
                ...
                sameSelectedGroupMatches = before,
                equivalentQuantFamilyMatches = 0,
                compositionMatches = rule.RuleType.Contains("Composition", StringComparison.OrdinalIgnoreCase) ? before : 0,
                contaminationSuppressedMatches = rule.RuleType.Contains("Contaminating", StringComparison.OrdinalIgnoreCase) ? before : 0,
                harmfulMatches = rule.RuleDirection.Contains("Harmful", StringComparison.OrdinalIgnoreCase) ? before : 0,
            ...
            AnsiConsole.MarkupLine(
                ...
                $"exactContextMatches=[cyan]0[/] sameSelectedGroupMatches=[cyan]{before:N0}[/] totalAdjustedRows=[cyan]{before:N0}[/]");
        }
    ...
    private static string BuildRuleWhere(AnomalyInteractionRule rule)
    {
        ...
        // Transferable counterfactual synergy template matching:
        // The exact Q8/Q6 dome remains strongest evidence, but application should not
        // require all surrounding groups to equal the discovery context. Match rows that
        // contain the selected group states, then compare them conceptually to a virtual
        // same-context twin where only those selected groups are raised back to ReferenceQuantId.
        // Surrounding groups are preserved by the virtual twin and therefore intentionally
        // not constrained here. Explicit probe/rule persistence remains strict elsewhere.
        foreach (var state in rule.GroupStates.OrderBy(x => x.SortOrder))
        {
            ...
            predicates.Add($"(CASE WHEN {column} = 0 THEN BaseQuant ELSE CAST({column} AS INTEGER) - 1 END) = {state.CandidateQuantId}");
        }

        if (string.Equals(rule.RuleDirection, AnomalyRuleDirection.SuppressionOnly.ToString(), StringComparison.OrdinalIgnoreCase))
            return string.Empty;
