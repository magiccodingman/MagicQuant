...
public sealed class ReadmeGenerationService
{
    ...
    {
        var replacementMap =
            FinalReleaseMetadataService.BuildReplacementMap(eliminatedBaselines ??
                                                            Array.Empty<BaselineEliminationRecord>());
        ...
        double? referencePpl =
            ResolveReferencePpl(pplReference, pureBaselineSnapshots, exportedArtifacts.Select(x => x.Snapshot));
        ...
        var rows = exportedArtifacts
            ...
            .Select(artifact =>
            {
                ...
                return new ReadmeArtifactRow
                {
                    ...
                    PplDeltaPercent =
                        FinalReleaseMetadataService.CalculatePplDeltaPercent(artifact.Snapshot.Ppl, referencePpl),
        ...
        CancellationToken ct = default)
    {
        ...
        var rows = records
            ...
            .Select(record =>
            {
                ...
                return new ReadmeArtifactRow
                {
                    ...
                    QuantFamily = string.IsNullOrWhiteSpace(artifact.QuantFamily)
                        ? artifact.BaseQuant
        ...
        var cloneContext = new ReadmeCloneContext
        {
            ...
            ArchivedManifestFileNames = archivedManifestFileNames?.Where(x => !string.IsNullOrWhiteSpace(x))
        ...
        return await GenerateCoreAsync(
            ...
            modelName,
            rows,
            hasReplacementDetails: cloneContext.ArchivedManifestFileNames.Contains(
                MagicQuantManifestPathService.ReplacementsFileName, StringComparer.OrdinalIgnoreCase),
        ...
        CancellationToken ct)
    {
        ...
        sb.AppendLine("<summary>Support MagicQuant</summary>");
        sb.AppendLine();
        sb.AppendLine(
        ...
        sb.AppendLine("<details>");
        sb.AppendLine("<summary>Warning - Is MagicQuant Better? (hint: how you frame the question matters)</summary>");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("So, MagicQuant can confidently tell you, \"under the same quantization to tensor configurations and identical imatrix, with this benchmark, I deemed this a winner\".");
        sb.AppendLine();
        sb.AppendLine("</details>");
        sb.AppendLine();
        //}
    ...
    private static string BuildCloneNameCell(string rawName, string fileName,
        IReadOnlyDictionary<string, IReadOnlyList<string>> replacementHints)
    {
    ...
    private static bool TryGetReplacementHint(IReadOnlyDictionary<string, IReadOnlyList<string>> replacementHints,
        string? key, out IReadOnlyList<string> replaced)
    {
    ...
    private static Dictionary<string, IReadOnlyList<string>> LoadCloneReplacementHints(string outputDirectory)
    {
        ...
        string path = MagicQuantManifestPathService.GetManifestFilePath(outputDirectory,
            MagicQuantManifestPathService.FinalSurvivorsFileName);
        if (!File.Exists(path))
            return output;
        ...
        catch (Exception ex)
        {
            AnsiConsole.MarkupLine(
                $"[yellow]Could not read clone replacement hints from archived final-survivors JSON:[/] {Markup.Escape(ex.Message)}");
        }
    ...
    private static void AddReplacementHint(Dictionary<string, IReadOnlyList<string>> output, string? key,
    ...
    private static void AppendReleaseMetadata(StringBuilder sb, ReadmeCloneContext? cloneContext)
    {
        ...
        if (ShouldLinkManifestFile(cloneContext, MagicQuantManifestPathService.HybridMapFileName))
            sb.AppendLine(
                $"- [Hybrid tensor map]({MagicQuantManifestPathService.HuggingFaceResolvePath(MagicQuantManifestPathService.HybridMapFileName)}) — tensor-group assignments and effective-state details for MagicQuant hybrid GGUFs.");

        sb.AppendLine(
            $"- [Clone tensor configs]({MagicQuantManifestPathService.HuggingFaceResolvePath(MagicQuantManifestPathService.CloneConfigsFileName)}) — exact per-GGUF tensor quantization maps for reproducing this final output list in repository clone mode.");

        if (ShouldLinkManifestFile(cloneContext, MagicQuantManifestPathService.IsolationSamplesFileName))
            sb.AppendLine(
                $"- [Isolation samples]({MagicQuantManifestPathService.HuggingFaceResolvePath(MagicQuantManifestPathService.IsolationSamplesFileName)}) — isolated base/group probe samples with KLD, PPL, PPL delta %, and size truth.");

        if (ShouldLinkManifestFile(cloneContext, MagicQuantManifestPathService.BadTradesFileName))
            sb.AppendLine(
                $"- [Bad trade details]({MagicQuantManifestPathService.HuggingFaceResolvePath(MagicQuantManifestPathService.BadTradesFileName)}) — structured bad-trade pruning decisions from the isolation optimizer.");

        if (cloneContext != null)
            sb.AppendLine(
                $"- [Clone benchmark summary]({MagicQuantManifestPathService.HuggingFaceResolvePath(MagicQuantManifestPathService.CloneBenchmarksFileName)}) — fresh benchmark results from this clone run.");
    ...
    private static bool ShouldLinkManifestFile(ReadmeCloneContext? cloneContext, string fileName)
    {
        return cloneContext == null ||
               ...
               string.Equals(fileName, MagicQuantManifestPathService.CloneConfigsFileName,
                   StringComparison.OrdinalIgnoreCase) ||
               string.Equals(fileName, MagicQuantManifestPathService.CloneBenchmarksFileName,
                   StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static bool NeedsYamlQuotes(string text)
    {
        ...
        char first = text[0];
        return first is '-' or '?' or ':' or '@' or '!' or '&' or '*' or '[' or ']' or '{' or '}' or '|' or '>' or '%'
            or '`' or ',';
    }
        ...
        FinalArtifactNamingContext namingContext)
    {
        ...
        return _namingService.ToPublicArtifactShortName(
            ...
            snapshot.IsHybrid
                ? "MagicQuant"
                : HybridBenchmarkRepository.ResolveProviderName(snapshot.Quant, exportNaming: false),
    ...
    private static void AppendReasonCodeDetails(StringBuilder sb)
    {
        ...
        sb.AppendLine(
            $"Underlined names in the table replaced or ultimately inherited the replacement of another artifact. Hover the name for the short replacement summary, or inspect `{MagicQuantManifestPathService.RelativeManifestPath(MagicQuantManifestPathService.ReplacementsFileName)}` for exact KLD/PPL/size deltas.");
        sb.AppendLine();
        sb.AppendLine("</details>");
    }
    ...
    private static string ToGB(ulong bytes) =>
        (bytes / 1000d / 1000d / 1000d).ToString("0.00", CultureInfo.InvariantCulture);

    private static string EscapePipe(string value) => (value ?? string.Empty).Replace("|", "\\|");

    private static string EscapeTooltip(string value) =>
