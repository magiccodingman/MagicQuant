    ...
    public async Task<string> DownloadBaselineAsync(BaselineQuants baseline, string destinationPath, bool forceRedownload = false, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (!PathsReferToSameLocation(downloadedPath, destinationPath))
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Refusing to delete Hugging Face staging artifact outside the external baseline cache: {downloadedPath}");
                }

                await HardDeleteHelper.DeleteFileIfExistsAsync(downloadedPath);
            }
